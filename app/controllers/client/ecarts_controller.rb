class Client::EcartsController < ApplicationController
  ...
  def add_to_cart
    ...
    return unless quantity > 0 and @product.existence >= quantity
    @quantity_is_correct = true
    ...
    if session[:e_cart][@product.hash_id]
      new_quantity = quantity - session[:e_cart][@product.hash_id].to_i
      added_weight = @product.Product.total_weight * new_quantity
    else
      added_weight = @product.Product.total_weight * quantity
    end

    # add product and quantity to session
    session[:e_cart][@product.hash_id] = quantity.to_s
  ...
  def remove_from_cart
    render_404 and return unless session[:e_cart]
  ...
  def update_quantity
    ...
    render_404 and return unless session[:e_cart].has_key?(params[:product]) and params[:new_quantity].to_i > 0
